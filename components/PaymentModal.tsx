  ...
  const handleCheckPayment = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('check-abacate-payment', {
        body: { pixId, dependentId, membershipId, transactionId }
      });

      if (error) throw error;

      if (data.status === 'PAID') {
        addToast('success', isRenewal ? 'Renovação anual confirmada com sucesso!' : 'Filiação anual confirmada com sucesso!');
        onSuccess();
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-slate-800 w-full max-w-md max-h-[90vh] overflow-y-auto scrollbar-hide rounded-3xl shadow-2xl relative border border-gray-100 dark:border-gray-700">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors z-10">
          <X size={24} />
        </button>
        ...
        <div className="p-8">
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-2">
              {isRenewal ? 'Renovação da Anuidade' : 'Pagamento da Anuidade'}
            </h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">
              {isRenewal ? 'Confirme o pagamento para renovar sua filiação por mais um ano.' : 'Escaneie o QR Code abaixo para finalizar sua filiação inicial.'}
            </p>
            ...
            <div className="w-full space-y-6">
              ...
              <div className="space-y-3">
                <button onClick={handleCopy} className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-3 transition-all ${copied ? 'bg-green-100 text-green-700 border-2 border-green-200' : 'bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-white border-2 border-transparent hover:bg-gray-200 dark:hover:bg-slate-600'}`}>
                  {copied ? <Check size={20} /> : <Copy size={20} />}
                  {copied ? 'Código Copiado!' : 'Copiar Código PIX'}
                </button>

                <button onClick={handleCheckPayment} disabled={isVerifying} className="w-full py-4 bg-cbjjs-blue hover:bg-blue-800 text-white rounded-xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 shadow-lg shadow-blue-900/20 active:scale-95 transition-all disabled:opacity-70">
        ...
        <div className="p-4 bg-gray-50 dark:bg-slate-700/50 flex items-center justify-center gap-1.5">
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Pagamento Seguro • AbacatePay</p>
        </div>
      </div>
    </div>
  );
