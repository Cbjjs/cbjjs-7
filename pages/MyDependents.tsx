                        ...
                        <div className="p-10 md:p-14">
                            ...
                            {step === 4 && (
                                <div className="space-y-8 animate-fadeIn">
                                    ...
                                    <div className="max-h-60 overflow-y-auto border rounded-3xl divide-y bg-gray-50/50">{loadingAcademies ? <div className="p-4 text-center"><Loader2 className="animate-spin inline mr-2"/> Carregando...</div> : academiesList.map(ac => (<button key={ac.id} onClick={() => setFormData({...formData, academyId: ac.id, academyName: ac.name})} className={`w-full p-5 text-left transition-all flex items-center justify-between ${formData.academyId === ac.id ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-50'}`}><span className="font-bold text-sm">{ac.name} — {ac.ownerName ? `Prof. ${ac.ownerName}` : 'Professor não informado'}</span>{formData.academyId === ac.id && <CheckCircle size={18}/>}</button>))}</div>
